...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['hero', 'teacher', 'organization', 'students'];
      const scrollPosition = window.scrollY + 120; 

      // 1. Cek apakah user sudah mentok di paling bawah halaman
      const isAtBottom = (window.innerHeight + window.scrollY) >= document.documentElement.scrollHeight - 10;

      if (isAtBottom) {
        setActiveSection('students');
      } else {
        // 2. Gunakan loop untuk mendeteksi section yang sedang dilihat
        for (const id of sections) {
          ...
          if (element) {
            ...
            if (scrollPosition >= offsetTop && scrollPosition < offsetTop + height) {
              setActiveSection(id);
              break; // Berhenti agar tidak loncat ke section di bawahnya
            }
          }
        }
      }
  ...
  const getNavClass = (id: string, isMobile: boolean = false) => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="relative w-10 h-10 grid grid-cols-1 grid-rows-1">
              <img 
                ...
                className={`col-start-1 row-start-1 w-full h-full object-contain transition-all duration-500 ${isScrolled ? 'opacity-0 scale-75' : 'opacity-100 scale-100'}`} 
              />
              <img 
                src={CLASS_DATA.logo} 
                className={`col-start-1 row-start-1 w-full h-full object-contain rounded-full transition-all duration-500 ${isScrolled ? 'opacity-100 scale-100' : 'opacity-0 scale-75'}`} 
              />
            </div>
          ...
          <div className="hidden md:flex items-center space-x-1">
            <a href="#hero" className={getNavClass('hero')}>Beranda</a>
            <a href="#teacher" className={getNavClass('teacher')}>Wali Kelas</a>
            <a href="#organization" className={getNavClass('organization')}>Struktur</a>
            <a href="#students" className={getNavClass('students')}>Murid</a>
          </div>
      ...
      {/* Mobile Menu Layout: Memanjang ke bawah */}
      <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${
        isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'
      }`}>
        <div className="bg-[#020617] border-t border-white/5 p-4 space-y-2 shadow-2xl">
          <a href="#hero" onClick={() => setIsOpen(false)} className={getNavClass('hero', true)}>Beranda</a>
          <a href="#teacher" onClick={() => setIsOpen(false)} className={getNavClass('teacher', true)}>Wali Kelas</a>
          <a href="#organization" onClick={() => setIsOpen(false)} className={getNavClass('organization', true)}>Struktur</a>
          <a href="#students" onClick={() => setIsOpen(false)} className={getNavClass('students', true)}>Murid</a>
        </div>
      </div>
    </nav>
  );
